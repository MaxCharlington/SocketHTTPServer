...
public:
    ...
    Request(Method method, string_like auto&& uri, Params params)
        : m_method{method}, m_uri{std::forward<decltype(uri)>(uri)},
        m_params{params}, m_protocol{protocol_versions[1]} {}
    ...
    Request(Method method, string_like auto&& uri)
        : m_method{method}, m_uri{std::forward<decltype(uri)>(uri)},
        m_params{}, m_protocol{protocol_versions[1]} {}
    ...
    [[nodiscard]] auto getUri() const -> Ret { return m_uri; }
...
[[nodiscard]] std::string Request::getStartLine() const {
    ...
    return http::toString(m_method) + ' ' + path_params + ' ' + m_protocol;
}
